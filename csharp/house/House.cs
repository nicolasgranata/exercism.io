...
{
    private static readonly Dictionary<int, string> verseDictionary = new Dictionary<int, string>
        {
    ...
    public static string Recite(int verseNumber) => GetVerse(verseNumber);

    public static string Recite(int startVerse, int endVerse) => GetVerse(startVerse, endVerse);

    private static string GetVerse(int startVerse, int endVerse = 0)
    {
        if (endVerse == 0)
        {
            return verseDictionary[startVerse];
        }
        ...
        {   
            if (i < endVerse)
            {
                builder.Append($"{verseDictionary[i]}\n");
            }
            else
            {
                builder.Append(verseDictionary[i]);
